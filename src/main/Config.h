//Wifi station name:
const char* ssid = "Apart-U";

//Wifi Password:
const char* password = "Apofiss.29";

//MQTT broker: leave as-is for Adafruit IO
const char* mqtt_server = "io.adafruit.com";

//Your Adafruit Username:
const char* mqttuser = "minimal88";

//Your secret AIO key:
const char* mqttpwd = "ac2f02cf5ec04acdaf57e6b9f8fb5940";

//URL containing <Name of your Maker Event> and <Your maker channel key>
//Note don't include the < and >
char iftttMakerUrl[] = "/trigger/<Name of your Maker Event>/with/key/<Your Maker Channel Key>";
